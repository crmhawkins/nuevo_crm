            ...
            <div class="col-12 col-md-6 order-md-2 order-first">
                <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="{{route('gastos-asociados.index')}}">Gastos Asociados</a></li>
    ...
    <section class="section mt-4">
        <div class="row">
            <div class="col-9">
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('gasto-asociado.update', $gasto->id) }}" method="POST">
                            @csrf
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select" id="purchase_order_id" name="purchase_order_id">
                                        ...
                                        @if (count($purchaseOrders) > 0)
                                            @foreach($purchaseOrders as $order)
                                                <option value="{{ $order->id }}" {{ $order->id == $gasto->purchase_order_id ? 'selected' : '' }}>{{ $order->id }} - {{ $order->concepto->concept  ?? ''}} - {{ $order->concepto->total ?? ''}} €</option>
                                            @endforeach
                                        @endif
                                    </select>
