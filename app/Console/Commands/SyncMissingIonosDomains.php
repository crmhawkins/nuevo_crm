...
class SyncMissingIonosDomains extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'ionos:sync-missing 
     ...
     * @var string
     */
    protected $description = 'Sincroniza dominios de IONOS que no existen en la tabla local';
    ...
    public function handle()
    {
        ...
        $this->info('🔄 Sincronizando dominios faltantes de IONOS...');
        ...
        if ($defaultClientId) {
            ...
            if ($client) {
                $this->line("  - Cliente por defecto: {$client->name} (ID: {$defaultClientId})");
            } else {
        ...
        try {
            ...
            $this->line("  - Dominios faltantes: {$missingCount}");
            
            if (!$dryRun) {
                $this->line("  - Dominios creados: {$createdCount}");
                $this->line("  - Errores: {$errorCount}");
            } else {
                $this->line("  - Modo DRY RUN: No se crearon dominios");
            }
            
            if ($missingCount > 0 && !$dryRun) {
                $this->info('✅ Sincronización completada. Se crearon ' . $createdCount . ' dominios nuevos.');
            } elseif ($missingCount > 0 && $dryRun) {
                $this->info('🔍 Se encontraron ' . $missingCount . ' dominios faltantes. Usa --dry-run=false para crearlos.');
            } else {
            ...
            Log::error('Error en SyncMissingIonosDomains: ' . $e->getMessage());
        }
    }
